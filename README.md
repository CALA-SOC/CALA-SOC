# Hello, I'm Clement
<a href="https://www.linkedin.com/in/emeka-clement-093098235"><img src="https://img.shields.io/badge/-LinkedIn-0072b1?&style=for-the-badge&logo=linkedin&logoColor=white" /></a>

📍 **Location**: Lagos, Nigeria  (Open for Relocation)  
💼 **Cybersecurity Enthusiast | SOC Level 1 Analyst |**              
📧 **Email**: clementemekalucky@gmail.com  

-----------------------
## 🛡️About Me
Aspiring SOC LV1 Analyst with hands-on experience in log analysis, alert triaging, and incident response. Google Cybersecurity and TryHackMe SOC LV1 Certified.

----------------------

## Objective

A cybersecurity analyst seeking an entry-level SOC Analyst role to begin a career in security operations. Equipped with foundational knowledge in log analysis, alert triaging, incident response, and Documentation through hands-on labs and certifications. Eager to learn, grow, and contribute to identifying and responding to cyber threats in a real-world environment.

---------------------
## 🛠️ Skills & Expertise

- **Security Monitoring & Incident Response**
  - Threat detection and alert triaging
  - Log analysis and correlation
  - Investigating suspicious activities

- **Security Tools & Platforms**
  - Splunk (forwarder configuration, search queries, dashboards)
  - Sysmon (installation, event monitoring)
  - Wireshark & Network Traffic Analysis
  - Windows Event Logs & Linux logs

- **Operating Systems**
  - Windows Server & Client (Active Directory, GPOs)
  - Linux (user management, log monitoring)

- **Scripting & Automation**
  - PowerShell (log collection, automation tasks)
  - Bash scripting (Linux monitoring)

- **Cybersecurity Knowledge Areas**
  - Security Operations Center (SOC) fundamentals
  - Network security and endpoint monitoring
  - Threat hunting & vulnerability assessment
 
----------------------------------
## 📜 Certifications
-  CompTIA Security + (Ongoing)

- 🛡️ TryHackMe SOC Level 1 Analyst  
  [View Certificate](https://github.com/CALA-SOC/cybersecurity-certificates/blob/main/TryHackMe%20SOC%20Level%201%20Certificate.pdf)

- 🔐 Google Cybersecurity Certificate  
  [View Certificate](https://github.com/CALA-SOC/cybersecurity-certificates/blob/main/Google%20Cybersecurity%20Certificate.pdf)


-----------------------------------
## Projects
- SOC Home Lab Project (VirtualBox + Splunk + Active Directory) <a href="https://github.com/CALA-SOC/Soc-home-lab">Soc Home Lab</a>
